...
public class CardEffectHelper {
    ...
    private static void applyInternal(MaterialCardView cardView, View shimmer, String id, String imageUrl,
        ...
        cardView.post(() -> {
            ...
            if (forcedGlowColor != null) {
                cardView.setStrokeColor(forcedGlowColor);
                ...
                if (parent != null) {
                    ...
                    float glowRadius = w * 0.12f;
                    float extraPadding = glowRadius * 2.0f;

                    View pseudoGlow = null;
                    if (applyGlow) {
                        pseudoGlow = new OuterGlowView(context, forcedGlowColor, cornerRadius, glowRadius, extraPadding);
    ...
    public static void applyEmptyStateGlow(MaterialCardView cardView, boolean applyFloating) {
        ...
        cardView.post(() -> {
            ...
            // Màu glow tím nhạt mặc định của hệ thống Mosco -> nay là lg_accent_primary_alpha_25
            int glowColor = ContextCompat.getColor(context, R.color.lg_accent_primary_alpha_25);

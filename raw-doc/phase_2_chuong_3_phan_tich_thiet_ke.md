...
Thay vì liệt kê đơn thuần, các luồng chức năng quan trọng nhất được đặc tả chi tiết nhằm làm rõ luồng sự kiện (Event Flow) giữa Tác nhân (Client) và Hệ thống (Server).
...
**Bảng 3.2: Đặc tả Use-case "Nâng cấp thẻ bài (FO4 Style)"**
| Thuộc tính | Mô tả chi tiết |
| :--- | :--- |
| **Mã UC** | UC-02 |
| **Tên Use-case** | Nâng cấp thẻ bài (FO4 Style) |
| **Tác nhân** | Người chơi (User) |
...
*[CHÈN HÌNH 3.2: Sơ đồ Tuần tự luồng Đăng nhập (Sequence Diagram). Trực quan: Client (App) gửi Username/Pass -> Auth Server -> Database kiểm tra -> Gen JWT Token -> Trả JWT về Client -> Client lưu vào Encrypted Shared Preferences để dùng cho các request sau]*
...
*[CHÈN HÌNH 3.5: Sơ đồ ERD Database. Trực quan: Vẽ 4 bảng chính `users`, `master_cards`, `user_cards`, `gacha_history` có nối các đường chỉ tuyến quan hệ 1-N]*
...
### 3.4. Thiết kế Giải thuật cốt lõi (Algorithm Pseudocode)

**3.4.1. Giải thuật Cắt ảnh Avatar (Face Crop) bằng Edge ML**
...
**3.4.2. Giải thuật Nâng cấp thẻ (Cơ chế Xác suất phi tuyến tính FO4)**
Giải thuật được thiết kế để xử lý bài toán rủi ro. Việc dùng Phôi xịn (OVR cao) đập vào Thẻ chính (OVR thấp) sẽ mang lại tỉ lệ thành công cao, và ngược lại.
```java
Hàm Nang_Cap_The(The_Chinh, Danh_Sach_Phoi):
    1. Ovr_Chinh = The_Chinh.get_Hien_Tai_Ovr()
    2. Tong_Ovr_Phoi = Tính_Tổng(Phoi.get_Hien_Tai_Ovr() CHO MỖI Phoi TRONG Danh_Sach_Phoi)
    
    // Tính toán Tỉ lệ thành công (Ví dụ tối đa 100%)
    3. Ty_Le_Cơ_Bản = 10.0%
    4. He_So_Bù_Đắp = (Tong_Ovr_Phoi / Ovr_Chinh) * 15.0%
    5. Ty_Le_Thang_Hoa = Min(Ty_Le_Cơ_Bản + He_So_Bù_Đắp, 100.0%)
    
    // Đổ xúc xắc
    6. Ti_So_Ngau_Nhien = Sinh_Số_Từ_0_Đến_100()
    7. NẾU Ti_So_Ngau_Nhien <= Ty_Le_Thang_Hoa:
        The_Chinh.tangLevel(+1)
        Trả về TRANG_THAI_THANH_CONG
    8. NGƯỢC LẠI:
        NẾU The_Chinh.getLevel() > 1:
            // Phạt rớt cấp độ nếu đập xịt (Tạo tính kịch tính cho Game)
            The_Chinh.giamLevel(-1) 
        Trả về TRANG_THAI_THAT_BAI
```

### 3.5. Danh mục API Endpoints cốt lõi
Hệ thống giao tiếp máy chủ tuân thủ nghiêm ngặt tiêu chuẩn RESTful API, sử dụng định dạng dữ liệu `application/json` và cơ chế xác thực Authorization Header mang Bearer JWT Token.

| Phương thức | URI Endpoint (Đường dẫn) | Chức năng nghiệp vụ & Ghi chú kỹ thuật |
| :--- | :--- | :--- |
| **POST** | `/api/auth/login` | Xác thực thông tin người dùng, sinh JWT Token. Hỗ trợ đa cơ chế đăng nhập (Email/Social). |
| **GET** | `/api/v1/cards/sync` | Cơ chế **Delta Sync**. Đòi hỏi tham số `lastSyncTime`. Server dùng tham số này để chỉ trả về các thẻ bài có sự thay đổi, tối ưu băng thông di động. |
| **POST** | `/api/gacha/spin/{packId}` | Giao dịch quay Gacha ngẫu nhiên. Gọi thuật toán TRNG, ghi log lịch sử và trao thẻ bài mới gói gọn trong một khối `@Transactional`. |
| **POST** | `/api/cards/upgrade` | Giao dịch Đập thẻ (Nâng cấp). Yêu cầu truyền lên `baseCardId` và mảng `materialIds`. Server thực thi **Pessimistic Lock** bảo vệ bản ghi. |
| **POST** | `/api/mailbox/claim-all` | Thu thập tất cả thư từ (Phần thưởng) bằng cơ chế Batch Update gom nhóm, giảm áp lực truy vấn MySQL. |
| **WS** | `/ws/chat` | Kênh kết nối Streaming WebSocket (chạy qua giao thức STOMP). Phục vụ tính năng Real-time Pub/Sub Chat toàn server. |

*(Lưu ý: Mọi API dạng GET trả về danh sách dữ liệu hiển thị (như Bảng xếp hạng, Lịch sử đấu) đều được Spring Boot phân trang (Pagination - `Pageable`) với mặc định 20 bản ghi/trang để chống tràn bộ nhớ Client).*
